...
## SuSiEx
```{r}
# TODO: update to read SuSiEx 1.1 results. --full_out is no longer an option, so take the max of the PIPs in all credsets for each SNP in the .snp file.
...
viewof xslider = Inputs.range([0,1], {label: "X slider"})
max_x = Math.pow(xslider,10)
viewof yslider = Inputs.range([0,1], {label: "Y slider"})
max_y = Math.pow(yslider,10)
viewof xyline = Inputs.toggle({label: "x=y line"})
