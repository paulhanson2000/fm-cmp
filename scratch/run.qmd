...
```{r}
#| output: false
ucscRefGenomeNaming <- function(config_file) {
...
lapply(list(data_config, loci_config, anno_config), ucscRefGenomeNaming)
...
### Load
```{r}
# TODO: paste0 filenames too wordy, predefine them or s/t to be more concise and avoid bugs
...
# TODO: stamp this gds filename w/ UUID or s/t.
...
### Filter sumstats using gnomAD ref panel
```{r}
ref <- seqOpen(paste0(misc_config$scratch_dir,"/gnomad.gds"))

ancestries_AFs <- sapply(unique(data_config$ancestry), simplify=F, function(anc) {
  if(anc=="EUR") anc <- "nfe" # TODO: not sure nfe or nfe+fin should be used as EUR
  seqGetData(ref, paste0("annotation/info/AF_",tolower(anc)))
}) 

sumstats <- mapply(sumstats, data_config$ancestry, SIMPLIFY=F, FUN = function(s, anc) {
    # TODO: filter by more things?
    gnomad_filter_passed <- seqGetData(ref, "annotation/filter") == "PASS"
    ancestry_AFs <- ancestries_AFs[[anc]]; AF_gt0_lt1 <- ancestry_AFs > 0 & ancestry_AFs < 1

    total_filter <- gnomad_filter_passed & AF_gt0_lt1
    seqSetFilter(ref, action="push+intersect", variant.sel = total_filter)
...
### Filter sumstats using 1kG ref panel
...
```{r}
# TODO: Could put this in the AWK preprocessing when first reading the file? But ugly. And would be unnecessary if user doesn't want annos.
...
# Iterative version TODO: rail-recursive is cool, but write this out just in case it turns out to be more concise/clearer
#mapply(sumstats, names(sumstats), SIMPLIFY=F, FUN=function(s,s_nm) {
...
## gnomAD
```{r}
# TODO: use seqApply + C for more speed
ld_matss <- lapply(loci_config$locus, function(l)) {
  seqSetFilterChrom() # TODO: for speed
  seqSetFilterAnnotID
  ld_mats <- lapply(unique(data_config$ancestry), function(anc) {
    rsids <- Reduce(union, lapply(subset_sumstatss[[l]], '[', j=rsid))
    ld_mat <- sapply(rsids, function(rs1) {
              sapply(rsids, function(rs2) {
      p_A <- seqGetData
      p_B <- 
  
})

# I want a vector of all the p_As and p_Bs so that can do nice concise vectorized code.
# How get that? seqSetFilterChrom(locus), then seqGetData(ref, "annotation/info/AF_<anc>")
  # But make sure is filtered to the rsids in the sumstats too after that.
    # Or better yet write that to file from the beginning using seqExport().
    # But can't do the same for 1kG b/c I have to respect that GDS file b/c it's raw data.
      # So might be inconsistent.
seqSetFilterAnnotID()
ld_matss <- lapply(1:nrow(loci_config), function(i) {

  loci <- loci_configs$hg38
  seqSetFilterChrom(ref, loci[i,chr],
                 from.bp=loci[i,pos_min],
                   to.bp=loci[i,pos_max])

  ld_mats <- lapply(unique(data_config$ancestry), function(anc) {
    if(anc=="EUR") anc <- "nfe" # TODO:
    anc <- tolower(anc)

    AFs <- seqGetData(ref, paste0("annotation/AF_",anc))
    #ANs <- seqGetData(ref, paste0("annotation/AN_",anc))
    #ACs <- seqGetData(ref, paste0("annotation/AC_",anc))
    #nhomalts <- seqGetData(ref, paste0("annotation/nhomalt_",anc))

    pABs <- AFs %o% AFs
    D <- 

    sapply(AFs, FUN=function(pA) {
    sapply(AFs, FUN=function(pB) {
      pAB <- pA*pB # NOTE: assuming HWE
      D <- pAB - pA*pB # uhhh...
      #Dmax <- ifelse(D<0, min(pA*pB, (1-pA)*(1-pB)),
      #                    min(pA*(1-pB), pB*(1-pA)))
      #Dprime <- D/Dmax
