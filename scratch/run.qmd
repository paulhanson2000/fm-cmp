...
sumstats <- lapply(1:nrow(data_configs), function(r) {
  ...
  # AWK command to give to fread(). Only loads the necessary variants from file, excluding those not in any locus from config.loci.
...
### TOPMed
```{r}
# want to filter by maf. but needd to request all info b4 that for topmed... which is a lot... wish coul ask only for maf, filter, THEN get rest of ata.
writeLines(sumstats[[1]]$rsid[1:100], "tmptopld.txt")
# TODO: ok this is gonna take 10mil years, already took idk over 5 mins w/ just 100 variants.
  # Instead, DL all the raw unfiltered INFO files for e/t, would be faster maybe? And then filter by MAF, and THEN request LD.
...
anno_colss <- mapply(sumstats, 1:length(sumstats), SIMPLIFY=F, FUN = function(sumstat,i) {
  ...
  writeLines(anno_configs$bed_file, "config.anno.tmp")

  if(!dir.exists("py_env")) system("virtualenv py_env; source py_env/bin/activate; pip3 install numpy")
  system(paste0("source py_env/bin/activate; python3 ../third_party/PAINTOR_V3.0/PAINTOR_Utilities/AnnotateLocus.py -o ~/scratch/anno_cols.txt -c chr -p pos -i config.anno.tmp -l ~/scratch/sumstat",i))
