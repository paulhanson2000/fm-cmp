...
# NOTE: don't worry if loci_configs' and anno_configs' ref_genome factor values are different for the same level label. They are still considered if compared across dataframes as you'd hope.
```
...
sumstats <- lapply(1:nrow(data_config), function(r) {
  ...
  chr_pos_eaf_preprocess_cmd <- paste0(
    "awk ",
    "-v chrs_string='",paste(collapse=' ', loci_config$chr),     "' ",
    "-v mins_string='",paste(collapse=' ', loci_config$pos_min), "' ",
    "-v maxs_string='",paste(collapse=' ', loci_config$pos_max), "' ",
    "'BEGIN {",
    ...
    "}",
    "{",
      "for(i=0; i<=",nrow(loci_config),"; i++) {",
        "if($",data_config[r,chr_col]," == chrs[i] && ",
           "$",data_config[r,pos_col]," >= mins[i] && ",
           "$",data_config[r,pos_col]," <= maxs[i] && ",
           "$",data_config[r,eaf_col]," >  0       && ", # TODO: make optional if user has no eaf col
           "$",data_config[r,eaf_col]," <  1) { ",
...
```{r}
#| output: false
seqSetFilterChrom(ref, loci_config$chr,
               from.bp=loci_config$pos_min,
                 to.bp=loci_config$pos_max)
...
anno_colss <- mapply(sumstats, 1:length(sumstats), SIMPLIFY=F, FUN = function(sumstat,i) {
  ...
  writeLines(anno_config$bed_file, "~/scratch/config.anno.tmp")
...
subset_sumstatss <- lapply(1:nrow(loci_config), function(r) {
  ...
  s[ chr == loci_config[r,chr]     &
     pos >= loci_config[r,pos_min] &
     pos <= loci_config[r,pos_max] ]
})})
