...
sumstats <- mapply(sumstats, data_config$ancestry, SIMPLIFY=F,
  FUN = function(s, anc) {
    ...
    seqSetFilter(ref, action = "push+intersect", variant.sel = ancestry_MAFs > 0.005) # TODO: Add MAF threshold to config.misc
...
# Add sample info. TODO: similarly to 1kG, move to a pre-process script?
...
seqSetFilter(ref, sample.sel = seqGetData(ref, "sample.annotation/ancestry") != "")
samples_of_each_ancestry <- tapply(seqGetData(ref,"sample.id"), INDEX=seqGetData(ref,"sample.annotation/ancestry"), FUN = identity) 
# NOTE: if it turns out you should just combine nfe and fin to make eur, then just union their sample ids here into a new eur col.
ancestries_MAFs <- lapply(samples_of_each_ancestry, function(samples_of_an_ancestry) {
    seqSetFilter(ref, sample.id = samples_of_an_ancestry)
    seqAlleleFreq(ref, minor=T)
}) 
...
sumstats <- mapply(sumstats, data_config$ancestry, SIMPLIFY=F, FUN = function(s, anc) {
    # TODO: Data ancestries: EAS EUR SAS. gnomad ancestry labels: eas nfe sas. More elegant way to handle this?
    if(anc=="EUR") anc <- "nfe"
    anc <- tolower(anc)

    # TODO: filter by more things?
    gnomad_filter_passed <- seqGetData(ref, "annotation/filter") == "PASS"
    ancestry_MAFs <- ancestries_MAFs[[anc]]; MAF_thresh_passed <- ancestry_MAFs > 0.005 # TODO: add MAF threshold to config.misc

    all_passed <- gnomad_filter_passed & MAF_thresh_passed
    seqSetFilter(ref, action="push+intersect", variant.sel = all_passed)
...
getBestFgwasModelP2 <- function(sumstat_filename_fgwas_compatible, annos) {
  # TODO: IDK the stats theory well enough: will the x-validation likelihoods here have only one minimum? If so, could optimize by doing a search w/ intelligently-chosen steps for the best penalty or s/t.
  # TODO: either way, could be good to focus down on a minimum a little more after the best one in steps of 0.05 is found.
...
getBestFgwasModelP3 <- function(sumstat_filename_fgwas_compatible, annos, xv_penalty, og_xv_llk) {
  # TODO: IDK the stats theory well enough: it says "keep dropping as long as likelihood keeps increasing", but should I drop the ones that give the most increase first?
