...
ld_calc <- function(gds_file, rs_ids, sample_ids=NULL, ref_alleles=NULL, outfile_name=NULL) {
  ...
  rs_ids_order <- match(seqGetData(gds_file,"annotation/id"), rs_ids)
  ld <- ld[rs_ids_order, rs_ids_order]

  if(!is.null(outfile_name)) fwrite(ld, outfile_name, sep=' ', col.names=F, compress="gzip")
...
ld_rsid_orders <-
  ...
  lapply(unique(data_configs$ancestry), function(anc) {
    ...
    common_rsids_in_sumstats_of_an_ancestry # Return the order of the LD mat's rows/cols, importnant! # TODO: write to file so no need recompute
})})
...
lapply(1:nrow(loci_configs), function(r) {
  system(paste("python3 ../third_party/SuSiEx/SuSiEx.py",
               ...
               "--maf 0.005", # TODO: won't work if set to 0? Confirm, and ask yourself why b/c haven't we filtered out all MAF==0?
               "--mult-step True",
               "--keep-ambig True",
               "--full_out True"
...
#  rs1   -3.14  -1.337
#  ... <no NAs allowed>
common_rsidss <- lapply(loci_configs$locus, function(l) {
  common_rsids <- Reduce(intersect, lapply(subset_sumstatss[[l]], '[', j=rsid))
  commonified_sumstats <- lapply(subset_sumstatss[[l]], function(s) s[rsid %in% common_rsids])
  # We sorted before, so all sumstats' rsids are in the same order even after subsetting by common rsids.
  paintor_locus_file <- Reduce(cbind, lapply(commonified_sumstats, '[', j=z), common_rsids) # Can add more metadata than just rsid if you want
...
tmp <- lapply(loci_configs$locus, function(l) {
  common_rsids <- Reduce(intersect, lapply(subset_sumstatss[[l]], '[', j=rsid))
  ...
  lds <- lapply(ld_filenames, fread)
  mapply(lds, ld_rsid_orders[[l]], desired_filenames, SIMPLIFY=F, FUN = function(ld, ld_rsid_order, nm) {
    new_order <- match(ld_rsid_order, common_rsids)
    new_order <- new_order[!is.na(new_order)]
    return(new_order)
    fwrite(ld[new_order,new_order,with=F], nm, sep=' ', col.names=F)
  })
...
lapply(loci_configs$locus, function(l) {
  common_rsids <- Reduce(intersect, lapply(subset_sumstatss[[l]], '[', j=rsid))
  writeLines(c("dummy_annot", rep(0,length(common_rsids))),
