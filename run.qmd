...
fgwasGetBestLikelihoodAnnos <- function(sumstat_file, annos_to_try, annos_acc, best_llk) {
  llks <- sapply(annos_to_try, function(anno_to_try) { # TODO: embarassingly parallel lapply
    fgwas_out_prefix <- paste0("out/fgwas/",anno_to_try,"-",basename(sumstat_file))

    model <- paste(collapse='+',c(annos_acc, anno_to_try))
    system(paste0("third_party/fgwas/src/fgwas -fine",
                  " -i ", sumstat_file,
                  " -w ", model, 
                  " -o ", fgwas_out_prefix))

    llk <- read.table(paste0(fgwas_out_prefix,".llk"))[1,2]
  })
