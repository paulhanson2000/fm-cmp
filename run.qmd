...
TODO: print out a bunch of "before and after" tables to make it clearer what the code is doing
...
# Load Configs
```{r}
misc_config <- sapply(yaml::yaml.load_file("config/misc.config"),identity)
data_config <- fread("config/data.config")
loci_config <- fread("config/loci-all.config")
anno_config <- fread("config/anno-plus_roadmap_and_tfs.config")
...
```{r}
# TODO: add a misc_config option to only do this if annotations are requested
...
if("fgwas" %in% misc_config$anno_method) {
  ...
  fgwas_results <-
  mapply(sumstats, names(sumstats), SIMPLIFY=F, FUN=function(s,s_nm) { # TODO: embarassingly parallel mapply
    ...
    message("Running fgwas on ",s_nm,"....")
    source("inc/fgwasFxns.R")
    # TODO: somewhat_signif_annos <- fgwasFirstPassorsomething(input_filename, anno_config$name, c(), 0, silent=T)
    # TODO: change below command to only use somewhat_signif_annos
    # TODO: Add aux/wrapper function so that c() and 0 don't have to be passed by the user?
    # TODO: Add fgwas_bin argument
    annos     <- fgwasGetBestLikelihoodAnnos(input_filename, anno_config$name, c(), 0, silent=T)
    l         <- fgwasGetBestXValidationPenalty(input_filename, annos, silent=T)
    best_annos <-fgwasGetBestXValidatedAnnos(input_filename, l$best_penalty, annos, l$best_xv_llk, silent=T)
    
    best_model <- paste(collapse='+', best_annos)
    return(best_model) # TODO: TODO: TODO: temporary, just to get the annotations to give to paintor
    system(paste("third_party/fgwas/src/fgwas -fine -print",
      "-i", input_filename,
      "-w", best_model,
      "-o", paste0("out/fgwas/final-",s_nm)))
  
    fread(paste0("out/fgwas/final-",s_nm,".bfs.gz"))
  })
...
ldCalc <- function(gds_file, rs_ids, sample_ids=NULL, ref_alleles=NULL, outfile_name=NULL, n_threads=parallel::detectCores()) {
  ...
  colnames(ld) <- rs_ids
  ld <- ld[!apply(ld,1,anyNA),!apply(ld,2,anyNA)] # TODO: Just removing them, but what is the source of these few remaining NaNs?

  if(!is.null(outfile_name)) {
    if(!dir.exists(dirname(outfile_name))) dir.create(dirname(outfile_name), recursive=T)
    fwrite(ld, outfile_name, sep=' ', compress="gzip")
  }
...
ldss <-
  ...
  sapply(unique(data_config$ancestry), simplify=F, function(anc) {
    ...
    ldCalc(ref, rs_ids = common_rs_ids_in_sumstats_of_an_ancestry,
...
#save(ld_rs_id_orders, file="in/ld/ld_rs_id_orders.RData") # TODO: Save as separate files, with one col of just rsIDs? This way, LD could be used by non-R programs more easily.
```
