...
execute:
  eval: false
  cache-comments: false # TODO not sure if this goes here under execute?
---
...
```{r display-configs, eval=T, echo=F, results="asis"}
small_tbl <- function(df) df %>% kable() %>% kable_styling() %>% scroll_box(height="200px")
...
```{r hash, eval=T}
#| code-fold: false
analysis_hash <- digest::digest(list(misc_config, data_config, loci_config, anno_config, ld_config, chr_nm_map), algo="md5")
```
Unique hash of the above config files: __`r analysis_hash`__\
If two runs of this pipeline have the same hash, then they used the exact same input.

## Hands-free hardcoded reference panels
```{r hardcoded-refpanels}
