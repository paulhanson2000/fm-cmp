...
```{r}
#| output: false
trade_speed_for_memory_efficiency <- F
...
sumstats <- lapply(1:nrow(data_config), function(r) {
  ...
  cols <- data_config[r, c( chr_col, pos_col, rsid_col, other_allele_col, effect_allele_col, eaf_col, b_col, se_col, z_col, p_col, n_col)]
  cols <- setNames(cols, c("chr",   "pos",   "rsid",   "a0",             "a1",              "eaf",   "b",   "se",   "z",   "p",   "n"   ))
  cols <- cols[!is.na(cols)]

  if(trade_speed_for_memory_efficiency) {

    # AWK command given to fread(). Filters out variants not in any config.loci locus.
    # If your data is huge but you only care to look at a few regions, this saves memory.
    # If your data is huge and you want to look at many regions (100+), awk will be much slower.
      # And if you are planning such a large analysis, you proably have a strong computer with enough memory anyways.
    # If your data is small, it doesn't matter.
    ...
    sumstat <- fread(cmd = chr_pos_preprocess_cmd, select = unname(cols))
    setnames(sumstat, names(cols))
  }
  else { # Speed! Filter ~after~ just reading the whole thing into memory.
...
if(misc_config$ref_panel == "1kG")
  ...
  ref_panel_files <- paste0("gs://gcp-public-data--gnomad/release/3.1.2/vcf/genomes/gnomad.genomes.v3.1.2.hgdp_tgp.chr",1:22,".vcf.bgz")
else
