...
ldss <-
  sapply(       loci_config$locus[1],     simplify=F, function(l) {
  ...
  sapply(unique(data_config$ancestry[1:3]), simplify=F, function(anc) { # TODO: [1:3] hardcoded
    ...
    outfile_name <- paste0(misc_config$scratch_dir,"ld_refpanelutil/",l,"-",anc,".ld.gz")
    fwrite(ld, outfile_name, sep=' ', compress="gzip")
    ld

    # TODO: add a bunch of LD-writing variations for each of the different methods
})})
