...
if(any(misc_config$ref_panel != "my_ld")) {
  ...
  if(tools::file_ext(misc_config$ref_panel[1]) == "gds") {
    ref_gds_file <- misc_config$ref_panel
  ...
  ref <- seqOpen(ref_gds_file)
}

# TODO: TODO:
# If ref panel has no rsids, then query for them.
  # How? bcftools directly, or bioconductor thingy
...
4. Are present in both sumstats and reference panel
```{r}
#| output: false
loci <- loci_configs[[misc_config$ref_panel_ref_genome]]

seqSetFilterChrom(ref, as.character(loci$chr), loci$pos_min, loci$pos_max, is.num=F)
...
anno_colss <- mapply(sumstats, 1:length(sumstats), SIMPLIFY=F, FUN = function(sumstat,i) {
  ...
  sumstat$chr <- paste0("chr",sumstat$chr) # TODO:TODO:TODO:
...
#save(sumstats, samples_of_each_ancestry, file="sumstats-filtered-annotated.RData")
```
