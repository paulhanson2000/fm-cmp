...
loci_config <- fread("config/loci.config")
...
```{r}
#| label: display-misc-config
#| cache: false
#| echo: false
cat(yaml::as.yaml(misc_config))
```
```{r}
#| label: display-other-configs
#| cache: false
#| echo: false
#| output: "asis"
...
```{r cache.path = paste0("cache/",config_hash,"/")}
#| label: hardcoded-refpanels
#| cache: true
if(!dir.exists(misc_config$scratch_dir)) { dir.create(misc_config$scratch_dir, recursive=T) }
...
We need versions of `loci.config` for the sumstats' and reference panel's build(s), to subset those big files to only the regions we need.\
...
Therefore, summary stats are lifted via rsIDs later (if their build is not already the same as the reference panel's). Here we lift only the *regions* in `loci.config` and `anno.config`.
...
#| label: ucsc-naming
#| cache: true
#| cache-vars: [data_config, loci_config, anno_config]
#| output: false
...
# TODO: would be good if it didn't turn already-properly-named chrs to NA. I.e. should be idempotent. The if statement is a messy fix.
if(!any(is.na(match(loci_config$chr,chr_nm_map$from)))) {
...
#| label: liftover
#| cache: true
#| dependson: ucsc-naming
out_dir <- paste0("out/",config_hash,"/liftover")
...
lapply(union(data_config$ref_genome, misc_config$ref_panel_ref_genome), function(dg) {
  ...
  fwrite(loci_config_lifted, paste0(out_dir, "/config.loci.",dg), sep=' ')
  loci_config_lifted
...
lapply(unique(data_config$ref_genome), function(dg) {
  ...
  sapply(1:nrow(anno_config), function(i) {
    ...
    if(dg != ag) {
      lifted_bed_file <- paste0(out_dir,"/",anno_config[i,name],"-lifted_to_",dg,".bed")
  ...
  fwrite(lifted_anno_config, paste0(out_dir, "/config.anno.",dg), sep=' ')
  lifted_anno_config
...
sumstats <- lapply(1:nrow(data_config), function(r) {
  ...
  if("eaf" %in% names(sumstat)) sumstat <- sumstat[eaf > misc_config$eaf_thresh & eaf < 1-misc_config$eaf_thresh]
  ...
  # TODO: maybe just compare w/ $chr $pos etc. directly instead of creating these IDs?
...
#| label: meta-sumstat
#| cache: true
meta <- data_config$ancestry=="META"
meta_sumstat <- sumstats[[which(meta)]]
sumstats <- sumstats[!meta]
...
if(!any(grepl("^rs", ref_panel_ids))) { # TODO: Actually can skip this if ref panel and ALL sumstats are of the same build. Can just use ch:pos:allele for e/t then.
  ...
  dbsnp_vcf <- {
    if(!dir.exists("data/dbsnp/")) dir.create("data/dbsnp")
    if      (misc_config$ref_panel_ref_genome == "hg19") "data/dbsnp/GCF_000001405.25.gz"
    else if (misc_config$ref_panel_ref_genome == "hg38") "data/dbsnp/GCF_000001405.40.gz"
    else stop("unsupported human genome version for dbsnp")
  }
...
```{r cache.path = paste0("cache/",config_hash,"/")}
#| label: filter-sumstats
#| cache: true
#| cache-vars: [sumstats, samples_of_each_ancestry]
#| output: false
...
sumstats <- mapply(sumstats, names(sumstats), data_config$ancestry, SIMPLIFY=F, FUN = function(s, s_nm, anc) {
  ...
  unshared <- s$rs_id %ni% seqGetData(ref, "annotation/id") # Done *after* filtering by mac/maf/miss, or else n_unshared would unfairly count variants dropped for other reasons. I.e. dropping due to bad mac/maf/miss is acceptable, but dropping lots of variants because unshared is more suspicious.
  message( sum(unshared),"/",nrow(s)," variants in ", s_nm, " will be omitted because they were not shared with the reference panel. Best p-values omitted: ", paste(head(sort(s[unshared,p]),n=5)) )
...
rsids_union <- Reduce(union, lapply(sumstats, '[', j=rs_id))
meta_sumstat <- meta_sumstat[rs_id %in% rsids_union]
```
...
if(!is.null(misc_config$anno_method)) {
  ...
  anno_cols <- {
    ...
    fwrite(meta_sumstat[, .(chr,pos)], paste0(scratch,"/fgwas_sumstat_input"), sep=' ')
    ...
    system(paste0("source ",py_env_dir,"bin/activate; ",
                  "python3 third_party/PAINTOR_V3.0/PAINTOR_Utilities/AnnotateLocus.py",
                    " -l ",scratch,"/fgwas_sumstat_input -c chr -p pos",
    ...
    unlink(paste0(scratch,"/fgwas_sumstat_input"))
    anno_cols 
  }

  meta_sumstat <- cbind(meta_sumstat, anno_cols)
}
```
...
#| label: fgwas
#| cache: true
#| cache-vars: [best_annos, fgwas_results]
if("fgwas" %in% misc_config$anno_method) {
  out_dir <- paste0("out/",config_hash,"/fgwas")
  if(!dir.exists(out_dir)) dir.create(out_dir, recursive=T)
  
  # Formats meta_sumstat in a way fgwas likes, then gets the annotations of the best-likelihood model.
  s <- meta_sumstat
  ...
  input_filename <- paste0(misc_config$scratch_dir,"/fgwas_input_sumstats.gz")
  ...
  if(length(annos)==0) { best_annos <- NULL; fgwas_results <- NULL; message("No annotations were significant.") }
  else {
    ...
    system(paste0("third_party/fgwas/src/fgwas-1000_iter -fine -print",
      " -i ", input_filename,
      " -w ", best_model,
      " -o ", out_dir,"/results"))
    
    fgwas_results <- fread(paste0(out_dir,"/results.bfs.gz"))
  }
}
```
...
```{r cache.path = paste0("cache/",config_hash,"/")}
#| label: subset-sumstats
#| cache: true
#| cache-vars: [subset_sumstatss, loci_configs, loci_config]
...
for(l in loci_config$locus) {
  if(all(sapply(subset_sumstatss[[l]],is.null))) {
    message("No summary stats for locus ",l," have any variants, at least not after filtering, so removing it.")
...
# LD, and Prepare Fine-Mapping Inputs
Calculate LD per ancestry per locus. And, while the LD is still in memory, write it in several different formats for the various fine-mapping methods.
```{r}
#| label: fm-input-aux-fxns
#| include: false
jobs_config <- fix_yaml(yaml::read_yaml("config/jobs.config"))
writeInputs <- function(jobs_config, locus, lds, subset_sumstats, all_loci_names) {
  sapply(names(jobs_config), function(method) {
    if(method=="paintor") writeInputsPaintor(<ARGS>)
    if(method=="susiex") writeInputsSusiex(<ARGS>)
  })
}

writeInputsPaintor <- function(locus_name, lds, subset_sumstats, p_thresh=1, all_loci_names) {
  in_dir <- paste0("in/",config_hash,"/paintor/")
  if(!dir.exists(in_dir)) dir.create(in_dir, recursive=T)
  # PAINTOR takes all loci at once, and requires that for each locus, each population have the same variants (rows) in the same order.
  # Thus if any variants are not shared between the ancestries, they unfortunately must be dropped.

  # input.files
  writeLines(all_loci_names, "input.files") # TODO: think about: right now all_loci_names is unique(loci_config$locus), but could specify in jobs.config to run on only a subset of the loci?

  # Locus files format:
  # rsid  pop1_z  pop2_z ...
  #  rs1   -3.14  -1.337
  #  ... <no NAs allowed>
  id_union <- Reduce(union, sapply(subset_sumstat,'[', i=p<p_thresh, j=rs_id))
  locus_file <- data.table(id_union)
  aligned_sumstats <- lapply(subset_sumstats, function(s) {
    s <- s[rs_id %in% id_union]
    s <- s[order(rs_id)]
    locus_file <<- cbind(locus_file, s$z)
  }) 
  fwrite(locus_file, locus_name) # TODO: write in a directory named by pval, since PAINTOR restricts the file name choice?
  stopifnot(length(unique(lapply(aligned_sumstats,'[',j=rs_id)))==1) # Sanity check: all rs_id columns are identical
  
  # Annotation files format:
  # annot1  annot2 ...
  # 0 or 1  0 or 1 
  # Rows ordered corresponding to locus file.
  TODO

  # LD format: Pearson r, space-delim'd, no header,no NAs, not compressed, rows ordered corresponding to locus file.
  lapply(lds, function(ld)) {
    inds <- which(colnames(ld) %in% id_unoin)
    fwrite(ld[inds,inds], TODO)
  }



  # TODO: Possible best_annos from fgwas
}

writeInputsSusiex <- function(locus_name, lds, subset_sumstats) {
  in_dir <- paste0("in/",config_hash,"/susiex/")
  if(!dir.exists(in_dir)) dir.create(in_dir, recursive=T)
  # Sumstat format: Requires chromosome, position, id, other allele, effect allele, effect size (must be named "BETA"), standard error, and p-value columns.
    # No need to make sure the variants are shared and ordered the same among all files, SuSiEx takes care of that for you using the IDs. # TODO: take a closer look at susiex's code to confirm
  # LD format: PLINK binary LD format https://github.com/getian107/SuSiEx/issues/7


  # Sumstats
  mapply(subset_sumstats, names(subset_sumstats), FUN=function(s, nm) {
    stopifnot(all(c("chr","pos","rs_id","a0","a1","b","se","p") %in% colnames(s))) # Just a sanity check; this should have been accounted for during input validation.
    s <- s[, .(chr,pos,rs_id,a0,a1,b,se,p)]
    setnames(s, old="b", new="BETA")
    fwrite(s, paste0(in_dir,locus_name,"-",nm), sep='\t')
  })

  # LD
  # TODO: Must develop a way to convert pre-computed LD from snpgdsLDMat output to PLINK binary LD format, or else can't use TOPMed LD...
    # Maybe daily note 230605 is still helpful?
}
```
...
sapply(loci_config$locus,       simplify=F, function(l)   {
  ...
  # For each locus ^, calculate an LD matrix for each ancestry. 
  lds <- sapply(unique(data_config$ancestry), simplify=F, function(anc) {
    ...
    outfile_name <- paste0(misc_config$scratch_dir,"ld/",l,"-",anc,"-",length(common_rs_ids_in_sumstats_of_an_ancestry),".ld.gz")
    if(file.exists(outfile_name)) return(as.matrix(fread(outfile_name)))
    ...
    ld <- ldCalc(ref, 
                 ...
                 n_threads = 6L)
    ld[is.na(ld)] <- 0
    ld <- round(ld, 5) # Save some disk space. (Actually more space-efficient than binary format below 12 digits! Default precision of R's 64b numeric is ~16 digits.)
    # It is okay to reduce the LD's precision because we are using the correlation of dosages LD approximation -- a lot of the precision is "fake" anyways.
  
    fwrite(ld, outfile_name, sep=' ', na="NA", compress="gzip")
    # TODO: check if imputing NAs is a big deal or not. If not sure, test how it affects the results vs. just rm'ing whole row.
      # If imputing to 0s is bad, then if NAs in a row/col, keep only the variant w/ the best p-val in meta_sumstat.
      # IN ADDITION, will have to RM the rm'd variants FROM THE SUMSTATS of that ancestry TOO, something like what I had before:
  ...
  # TODO:
  all_loci_names <- unique(loci_config$locus)
  writeInputPaintor(locus, lds, subset_sumstatss)
  #writeInputSusiex
...
# Run
out_dir <- paste0("out/",config_hash,"/susiex")
if(!dir.exists(out_dir)) dir.create(out_dir, recursive=T)
...
### Run PAINTOR
```{r eval=F}
out_dir <- paste0("out/",config_hash,"/paintor")
if(!dir.exists(out_dir)) dir.create(out_dir, recursive=T)
