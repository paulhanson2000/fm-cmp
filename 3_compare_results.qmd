---
title: Compare Fine-Mapping Methods' Results
---
...
```{r}
#| cache: true 
loci_configs <- fread("config/loci.config")
...
mahajan_results <- lapply(mahajan_results, function(mr) {
  ...
  setnames(mr, old=c("rsID"), new="rs_id")
})
```
...
paintor_results <- lapply(result_files, fread, select=c("rs_id", "Posterior_Prob"),
                                             col.names=c("rs_id",       "pip"     ))
...
resultss <- sapply(loci_configs$locus, simplify=F, function(l) {
  locus_rs_ids_union <- Reduce(union, lapply(resultss, function(results) results[[l]]$rs_id))
  lapply(resultss, function(results) {
    method_missing_rs_ids <- locus_rs_ids_union[locus_rs_ids_union %ni% results[[l]]$rs_id]
    if(length(method_missing_rs_ids)==0)
      tmp <- results[[l]]
    else
      tmp <- rbind(results[[l]], data.table(rs_id = method_missing_rs_ids, pip = 0))
    setorder(tmp, rs_id)
  })
